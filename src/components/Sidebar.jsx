...
const pages = {
  "About us": ["Our Company", "Team", "Review"],
  Home: ["Banner","Overview"],
  Service: ["Web Design", "Google Ads"],
};

export default function Sidebar({ onSelect, activePage, activeSubpage }) {
  return (
        ...
        <div key={page}>
          ...
          <ul className="mb-2">
            {subpages.map((sub) => (
              <li
                key={sub}
                onClick={() => onSelect(page, sub)}
                className={`cursor-pointer px-2 py-1 rounded hover:bg-gray-700 ${
                  activePage === page && activeSubpage === sub ? "bg-gray-700" : ""
                }`}
              >
                {sub}
              </li>
